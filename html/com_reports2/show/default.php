...
//----Monat-Parameter aus �bergabe auslesen----
...
if ($reportData)
{
  ...
  if (!$display) {
    ...
    //----Jahr Parameter aus Alarmierungsdatum f�r Link-Variabeln �bergabe vorbereiten
...
//----------------------------- URL-Daten f�r Twitterlink ---------------------------------------
        ...
        $url = $root.$url;
//--------------- Seitentitel und Beschreibung im Metadaten �ndern (f�r Facebook notwendig !!) --------------------
...
{
// Abfrage: Wechen Einsatz hatte eine Wehr X VOR dem momentan ausgw�hlten
$query = 'SELECT * FROM `#__reports` LEFT JOIN `#__reports_departments_link` ON #__reports_departments_link.report_id=#__reports.id   WHERE `date1` < "'.$rDate1.'"  AND `department_id` LIKE "'.$selectedDepartment.'" AND `#__reports`.`published`="1" ORDER BY `#__reports`.`date1` desc LIMIT 1' ;//## 110302 published hinzugef�gt
$db->setQuery($query);
$prev = $db->loadobject();
...
{
// Abfrage: Falls keine Wehr ausgew�hlt ist, dann wird der Einsatz der genau vor diesem Einsatz stattgefunden hat, ohne Beachtung der Wehrbeteiligung
$db =& JFactory::getDBO();
$query = "SELECT * FROM `#__reports` WHERE `date1` < '".$rDate1."' AND `published`='1' ORDER BY `#__reports`.`date1` DESC LIMIT 1";//## 110302 published hinzugef�gt
$db->setQuery($query);
$prev = $db->loadobject();
}
?>
...
if ($selectedDepartment != 0)
{
// Abfrage: Welchen Einsatz hatte eine Wehr X NACH dem momentan ausgw�hlten
$query = 'SELECT * FROM `#__reports` LEFT JOIN `#__reports_departments_link` ON #__reports_departments_link.report_id=#__reports.id   WHERE `date1` > "'.$rDate1.'"  AND `department_id` LIKE "'.$selectedDepartment.'" AND `published`="1" ORDER BY `#__reports`.`date1` asc LIMIT 1' ;//## 110302 published hinzugef�gt
$db->setQuery($query);
$next = $db->loadobject();
}
else
{
// Abfrage: Falls keine Wehr ausgew�hlt ist, dann wird der Einsatz der genau nach diesem Einsatz stattgefunden hat, ohne Beachtung der Wehrbeteiligung
$db =& JFactory::getDBO();
			$query = "SELECT * FROM `#__reports` WHERE `date1` > '".$rDate1."' AND `published`='1' ORDER BY `#__reports`.`date1` asc LIMIT 1"; //## 110302 published hinzugef�gt
			$db->setQuery($query);
			$next = $db->loadobject();
}			
if ($next->id != 0) {
...
// Anzahl der bisherigen Eins�tze im Jahr x
...
// Anzahl aller Eins�tze einer bestimmten Wehr in diesem Jahr
...
// Ausgabe welche Wehr im Men� verlinkt ist (um von der ID auf den Namen zu kommen)
...
<?php
// -------------------- COUNTER -------------------------------- ##110402 Counter->IP->SQL hinzugef�gt
$db =& JFactory::getDBO();
...
//echo $query;
 ...
 {
 // Counter wird f�r Ip-Adresse auf bestimme Zeit gesperrt
 }
 else
 {
// Ip-Adresse hat diesen Bericht noch nicht gelesen. Es folgt Counter-Z�hlung und Speicherung der IP f�r eine bestimmte Zeit
// au�erdem werden Zeit abgelaufende gesperrte Ip-Adressen wieder freigegeben.	 
$i=$count;
$i=$i+1;
global $option;
$db =& JFactory::getDBO();
